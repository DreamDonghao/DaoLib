            ...
            if (m_status == InputStatus::Input) {
                ...
                if (event.type == SDL_EVENT_TEXT_INPUT) {
                    ...
                    for (auto it = begin; it != end; ++it) {
                        m_str.insert(m_str.begin() + cursorPos, static_cast<char32_t>(*it));
                        cursorPos += 1;
                    }
                } else if (event.type == SDL_EVENT_KEY_DOWN) {
                    if (event.key.key == SDLK_BACKSPACE && !m_str.empty()) {
                        if (cursorPos != 0) {
                            m_str.erase(cursorPos - 1, 1);
                            --cursorPos;
                        }
                    } else if (event.key.key == SDLK_DELETE) {
                        if (cursorPos != m_str.size()) {
                            m_str.erase(cursorPos, 1);
                        }
                    } else if (event.key.key == SDLK_LEFT) {
                        cursorPos = std::max(cursorPos - 1, 0);
                    } else if (event.key.key == SDLK_RIGHT) {
                        cursorPos = std::min(cursorPos + 1, static_cast<int>(m_str.size()));
                    }
