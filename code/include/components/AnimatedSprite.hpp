...
namespace dao {
    ...
    class AnimatedSprite : ifc::IDrawable {
    public:
        /// @brief 构造函数（使用初始化列表）
        /// @param fps 动画帧率
        /// @param x 精灵左上角 x 坐标
        ...
        AnimatedSprite(const u32 fps, const f32 x, const f32 y, const f32 width, const f32 height,
                       const std::initializer_list<i32> &textureIDs)
            : m_tickTimer(std::chrono::milliseconds(1000 / fps)) {
            for (const auto textureID: textureIDs) {
                Image texture{x, y, x + width, y + height, textureID};
        ...
        template<size_t N>
        AnimatedSprite(const u32 fps, const f32 x, const f32 y, const f32 width, const f32 height,
                       const std::array<i32, N> &textureIDs)
            : m_tickTimer(std::chrono::milliseconds(1000 / fps)) {
            for (const auto textureID: textureIDs) {
                Image texture{textureID, x, y, x + width, y + height};
                m_textures.push_back(texture);
            }
        }
        ...
        /// @param end 结束纹理 ID
        AnimatedSprite(const u32 fps, const f32 x, const f32 y, const f32 width, const f32 height,
                       const i32 begin, const i32 end)
            : m_tickTimer(std::chrono::milliseconds(1000 / fps)) {
            for (i32 textureID = begin; textureID < end; ++textureID) {
                Image texture{x, y, x + width, y + height, textureID};
                m_textures.push_back(texture);
            }
        }
        ...
        void writeToBatch(BatchRenderer &batchRenderer) const override {
            m_textures.val().writeToBatch(batchRenderer);
            if (m_tickTimer) {
                m_textures.next();
            }
        }
    ...
    private:
        mutable RingVector<Image> m_textures; ///< 纹理循环队列
